...
class Template_ extends \stdClass
{
    ...
    public $compile_dir = false;
    public $compile_ext = 'php';
    public $skin = '';
    public $notice = false;
    public $path_digest = false;

    public $template_dir = false;
    public $prefilter = '';
    public $postfilter = '';
    ...
    public function __construct($config = [])
    {
        ...
        if ($this->template_dir === false) {
            $this->template_dir = BASEPATH . '../' . getenv('view_template_dir');
        }

        if ($this->compile_dir === false) {
            $this->compile_dir = BASEPATH . '../' . getenv('view_compile_dir');
        }
    }
    ...
    protected function _define($fid, $path)
    {
        switch ($fid[0]) {
            case '!':
                $type = 'txt';
        ...
        if ($file_type === 'tpl') {
            ...
            if ($sub) {
                ...
                if ($this->notice) {
                    ...
                    set_error_handler([$this, '_template_notice_handler']);
                    $this->_include_tpl($compile_path, $fid, $scope);
    ...
    protected function _include_tpl($TPL_CPL, $TPL_TPL, $TPL_SCP)
    {
    ...
    protected function _include_php($file_path)
    {
    ...
    protected function _print_txt($file_path)
    {
    ...
    protected function _get_compile_path($rel_path)
    {
    ...
    public function setCache($fid, $expiration = 1, $remover = '', $multiplier = '')
    {
    ...
    protected function _cache_enabled($fid)
    {
        return $this->caching && $this->tpl_[$fid]['cache'] && $this->compile_check !== 'dev';
    }

    public function isCached($fid)
    {
    ...
    protected function _empty_out_cache_dir($dir_path)
    {
    ...
    protected function _shell_exec_available()
    {
    ...
    protected function _empty_out_recursively($dir_path, $recursive = 0)
    {
        if (!$d = @dir($dir_path)) {
            return;
        }
    ...
    protected function _path_in_cache_remover($header)
    {
    ...
    protected function _touch_cache_remover($path)
    {
        if (@is_file($path)) {
            return;
        }
    ...
    protected function _make_cache_dir($path)
    {
    ...
    public function exit_($type, $msg)
    {
    ...
    public function xprint($file, $type = 'tpl')
    {
        $this->define(($type == 'txt' ? '!*' : '*'), $file);
        $this->print_('*');
    }

    public function xfetch($file, $type = 'tpl')
    {
        $this->define(($type == 'txt' ? '!*' : '*'), $file);
        return $this->fetch('*');
    }
    ...
    public function loopLoad($id, $n = 1)
    {
        if ($n === 1) $this->b1 = &$this->var_[$this->_current_scope][$id];
        else $this->{'b' . $n} =& $this->{'b' . --$n}[count($this->{'b' . $n}) - 1][$id];
    }

    public function loopPushAssign($arg, $n = 1)
    {
        $this->{'b' . $n}[] = $arg;
    }

    public function loopPushLoad($id, $n = 2)
    {
        $this->{'b' . $n} =& $this->{'b' . --$n}[][$id];
    }

    public function loopAssign($arg, $n = 1)
    {
        $section = array_merge($section =& $this->{'b' . $n}[count($this->{'b' . $n}) - 1], $arg);
    }

    public function setLoop($id, $arg, $n = 1)
    {
    ...
    public function pushSetLoop($id, $arg, $n = 2)
    {
        $this->{'b' . --$n}[][$id] = is_array($arg) ? $arg : array_pad(array(), $arg, 1);
    }

    protected function _template_notice_handler($type, $msg, $file, $line)
    {
